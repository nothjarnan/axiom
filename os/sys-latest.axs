...
-- New FS functions:
  _G.fs = {
    isReadOnly = function(f)
      --edge.log("Checked if "..f.." is read only")
      return oldFS.isReadOnly(f)
    end,
    combine = function(basepath, localpath)
      --edge.log("Combined "..basepath.." and "..localpath)
      return oldFS.combine(basepath, localpath)
    end,
    find = function(wildcard)
      --edge.log("Finding using wildcard "..wildcard)
      return oldFS.find(wildcard)
    end,
    list = function(f)
      --edge.log("Listing files in dir "..f)
      return oldFS.list(f)
    end,
    isDir = function(f)
      --edge.log("Checking if "..f.." is dir")
      return oldFS.isDir(f)
    end,
    exists = function(f)
      --edge.log("Checking existence of "..f)
      return oldFS.exists(f)
    end,
    copy = function(s,e)
      --edge.log("Copying "..s.." to "..e)
      return oldFS.copy(s,e)
    end,
    move = function(s,e)
      --edge.log("Moving "..s.." to "..e)
      return oldFS.move(s,e)
    end,
    getDir = function(path)
    --  edge.log("Getting dir "..path)
      return oldFS.getDir(path)
    end,
    getSize = function(file)
    --  edge.log("Got size of "..file)
...
function login_gui_unreleased()
  ...
  --         currentUser = key
  --         parallel.waitForAll(desktop  ,clock,checkForUpdates)
  --       else
  ...
  --           currentUser = key
  --           parallel.waitForAll(desktop  ,clock,checkForUpdates) --NOTE: DO NOT FUCKING CHANGE THIS YA TWATS
  --           return
  --         else
  --           login_gui_unreleased()
  --         end
...
function desktop()
    ...
    while(true) do
      ...
      if x >= 1 and x <= 4 and y == 1 and button == 1 then
        ...
        local mWidth = 20
        local mTitle = "Menu"
        edge.render(1,2,mWidth,10,menubarColor,colors.cyan,"",colors.white,true)
        edge.render((mWidth/2)-(string.len(mTitle)/2),2,mWidth,2,menubarColor,colors.cyan,mTitle,colors.black,false)
        edge.render(1,3,mWidth,3,menubarColor,colors.cyan,"Update",colors.black,false)
        if updating then edge.render(1,3,10,3,menubarColor,colors.cyan,"Updating..",colors.black,false) end
        edge.render(1,4,mWidth,4,menubarColor,colors.cyan,"Settings",colors.black,false)
        edge.render(1,5,mWidth,5,menubarColor,colors.cyan,"Files",colors.black,false)
        edge.render(1,6,mWidth,5,menubarColor,colors.cyan,"Terminal  ",colors.black,false)
        edge.render(1,7,mWidth,7,menubarColor,colors.cyan,"Logout",colors.black,false)
        edge.render(1,8,mWidth,8,menubarColor,colors.cyan,"Reboot",colors.black,false)
        edge.render(1,9,mWidth,9,menubarColor,colors.cyan,"Shut down",colors.black,false)
        edge.render(1,10,mWidth,10,menubarColor,colors.cyan,"Programs >",colors.black,false)
        while(true) do
          local event, button, x, y = os.pullEvent("mouse_click")
          if event == "terminate" then
            if edge.windowAlert(20,5, "Logout?",false) then
...
function sysInfo()
  ...
  if not oldFS.exists("Axiom/discoverAppStore.app") then
    error("discoverAppStore not found.")
  end
  ...
  for i,v in ipairs(f_file) do
    ...
    if oldFS.isDir(files..f_file[i]) then
      if f_file[i] == "rom" then
        ...
        if #l == 1 then
            edge.render(31,fy,41,fy,colors.white,colors.cyan,#l.." item",colors.black)
        else
...
function initialize()
  ...
  --login_gui()
  parallel.waitForAll(login_gui_unreleased,keyStrokeHandler )
